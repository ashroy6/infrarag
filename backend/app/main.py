...
from app.cancel_registry import cancel_request
...
def ask(
    ...
    try:
        ...
        logger.exception("Ask orchestration failed")
        raise InfraRAGError(str(exc), status_code=500, code="ask_orchestration_failed") from exc



@app.get("/citation")
def get_citation(source_id: str = "", chunk_index: int = -1):
    if not source_id:
        raise InfraRAGError("source_id is required", status_code=400, code="missing_source_id")

    if chunk_index < 0:
        raise InfraRAGError("chunk_index must be >= 0", status_code=400, code="invalid_chunk_index")

    citation = get_chunk_by_source_id_and_index(source_id, chunk_index)
    if not citation:
        raise InfraRAGError("Citation not found", status_code=404, code="citation_not_found")
...
def api_rename_conversation(conversation_id: str, payload: dict):
    title = (payload or {}).get("title", "").strip()
    if not title:
        raise InfraRAGError("title is required", status_code=400, code="missing_title")

    rename_conversation(conversation_id, title)
    return {"message": "Conversation renamed", "conversation_id": conversation_id, "title": title}
...
@app.post("/cancel")
def api_cancel_request(payload: dict):
    request_id = str((payload or {}).get("request_id") or "").strip()

    if not request_id:
        raise InfraRAGError("request_id is required", status_code=400, code="missing_request_id")

    cancel_request(request_id, reason="Cancelled by user from UI.")
